...
//        $base_price = $price / 2;
        ...
        try {
            ...
            $markethistoryexists = MarketHistory::where('market_id',$request->market)->where('user_id',auth()->user()->id)->where('is_first',1)->exists();
            $is_first = 0;
...
//            $message = 'Min quantity you can enter is: ' . $min_order . ' ' . $unit;
        ...
        if($bid_history_delete){
         $best_price = MarketHistory::where('is_deleted',0)->where('market_id',$market->id)->max('price');
