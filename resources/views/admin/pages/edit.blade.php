                                                ...
                                                <div class="col-12 col-md-4 mb-3">
                                                    ...
                                                    <select id="menu" name="menu" class="form-control">
                                                        ...
                                                        @foreach($menus as $menu)
                                                            <option {{ in_array($menu->id,$page->Menus()->pluck('id')->toArray()) ? 'selected' : '' }} value="{{ $menu->id }}">{{ $menu->title }}</option>
                                                            @foreach($menu->children as $child)
                                                                <option {{ in_array($child->id,$page->Menus()->pluck('id')->toArray()) ? 'selected' : '' }} value="{{ $child->id }}">{{ $menu->title.' - '.$child->title }}</option>
                                                            @endforeach
                                                        @endforeach
                                                    </select>
                                                    @error('menu')
